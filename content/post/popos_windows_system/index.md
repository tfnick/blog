---
title: 'popos与windows双系统安装与配置-支持单硬盘双系统，双硬盘双系统'
description: 'popos与windows双系统安装与配置-支持单硬盘双系统，双硬盘双系统'
summary: "popos与windows双系统安装与配置-支持单硬盘双系统，双硬盘双系统"
date: '2024-02-15'
author: '飞天'
usePageBundles: true
toc: true
...
**提示：与Ubuntu不一样，popOS不再使用Grub多系统引导，而是使用systemd-boot作为多系统引导。**
...
1. Dual boot using a single drive （多系统安装在同一个硬盘）
2. Dual boot using two drives with each OS on its own storage device（双系统安装在多个独立的硬盘）
3. Installing Pop!_OS first, then Windows（先安装Pop!__OS，再安装widows）
4. Installing Windows first, then Pop!_OS（先安装Windows，再安装Pop!__OS）
...
参考：https://github.com/spxak1/weywot/blob/main/Pop_OS_Dual_Boot.md